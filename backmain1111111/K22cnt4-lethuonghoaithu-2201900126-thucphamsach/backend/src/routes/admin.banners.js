...
router.post("/", upload.single("hinhanh"), (req, res) => {
    const { title, description, button_text, button_color, link_path, thutu, trangthai } = req.body;
    const hinhanh = req.file ? req.file.filename : null;

    const sql = "INSERT INTO banners (title, description, hinhanh, button_text, button_color, link_path, thutu, trangthai) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    db.query(sql, [title, description, hinhanh, button_text, button_color, link_path, thutu || 0, trangthai || 1], (err) => {
        if (err) return res.status(500).json({ error: err.message });
...
router.put("/:id", upload.single("hinhanh"), (req, res) => {
    ...
    db.query("SELECT hinhanh FROM banners WHERE ma_banner = ?", [id], (err, rows) => {
        ...
        const sql = "UPDATE banners SET title=?, description=?, hinhanh=?, button_text=?, button_color=?, link_path=?, thutu=?, trangthai=? WHERE ma_banner=?";
        db.query(sql, [title, description, newImage, button_text, button_color, link_path, thutu, trangthai, id], (err) => {
            if (err) return res.status(500).json({ error: err.message });
