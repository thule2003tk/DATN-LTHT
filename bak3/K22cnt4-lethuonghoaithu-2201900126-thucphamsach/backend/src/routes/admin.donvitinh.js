...
router.post("/", (req, res) => {
    ...
    console.log("🚀 Inserting Unit with ID:", ma_dvt);

    const sql = "INSERT INTO donvitinh (ma_dvt, ten_dvt, mota, size, trangthai) VALUES (?, ?, ?, ?, ?)";
    db.query(sql, [ma_dvt, ten_dvt, mota || null, size || null, trangthai || 'active'], (err) => {
        if (err) {
            console.error("❌ Link Query Error (POST donvitinh):", err);
            return res.status(500).json({
                error: "Lỗi thêm đơn vị tính",
                details: err.message,
                sql: err.sql,
                code: err.code
            });
